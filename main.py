...
FIRST_AVAILABLE_DATE_CACHE: dict[str, pd.Timestamp | None] = {}
YFINANCE_IO_LOCK = Lock()
...
def call_yfinance_silently(fn):
    with YFINANCE_IO_LOCK, warnings.catch_warnings():
...
def fetch_first_available_yahoo_date(symbol: str) -> pd.Timestamp | None:
    if symbol in FIRST_AVAILABLE_DATE_CACHE:
        return FIRST_AVAILABLE_DATE_CACHE[symbol]

    try:
        history = call_yfinance_silently(
            lambda: yf.Ticker(symbol).history(period="max", auto_adjust=False, actions=False)
        )
    except Exception:
        FIRST_AVAILABLE_DATE_CACHE[symbol] = None
        return None
    ...
    normalized_index = normalize_history_index(history.index)
    first_date = pd.Timestamp(normalized_index.min()) if len(normalized_index) else None
    FIRST_AVAILABLE_DATE_CACHE[symbol] = first_date
    return first_date


def describe_missing_stock_history(symbol: str, start: pd.Timestamp, end: pd.Timestamp) -> str:
    first_available = fetch_first_available_yahoo_date(to_yf_symbol(symbol))
    if first_available is None:
        return f"{symbol}: No trading data found in selected range."

    normalized_start = pd.Timestamp(start).normalize()
    normalized_end = pd.Timestamp(end).normalize()
    if first_available > normalized_end:
        return (
            f"{symbol}: Selected range {normalized_start.date()} to {normalized_end.date()} is entirely earlier "
            f"than the earliest available data {first_available.date()}."
        )
    if first_available > normalized_start:
        return (
            f"{symbol}: Start date is {normalized_start.date()}, but earliest available data is "
            f"{first_available.date()}. Please move the backtest start date forward."
        )
    return f"{symbol}: No trading data found in selected range."


def describe_missing_fx_history(start: pd.Timestamp, end: pd.Timestamp) -> str:
    first_available = fetch_first_available_yahoo_date("TWD=X")
    if first_available is None:
        return "Unable to fetch historical USD/TWD exchange rates."

    normalized_start = pd.Timestamp(start).normalize()
    normalized_end = pd.Timestamp(end).normalize()
    if first_available > normalized_end:
        return (
            f"Historical USD/TWD data on Yahoo Finance starts at {first_available.date()}, so the selected range "
            f"{normalized_start.date()} to {normalized_end.date()} is entirely earlier than available FX data. "
            "No fallback is applied because using a future FX rate would introduce look-ahead bias."
        )
    return (
        f"Historical USD/TWD data on Yahoo Finance starts at {first_available.date()}. "
        f"The selected start date {normalized_start.date()} is earlier than the earliest available FX date. "
        "No fallback is applied because using a future FX rate would introduce look-ahead bias."
    )
...
def download_stock_dividend_history(symbol: str, start: pd.Timestamp, end: pd.Timestamp) -> pd.Series:
    yf_symbol = to_yf_symbol(symbol)
    dividends = call_yfinance_silently(lambda: yf.Ticker(yf_symbol).dividends)
...
def fetch_stock_history(symbol: str, start: pd.Timestamp, end: pd.Timestamp) -> AssetHistory:
    ...
    if close is None or close.empty:
        raise ValueError(describe_missing_stock_history(symbol, start, end))
...
def fetch_usd_twd_history(start: pd.Timestamp, end: pd.Timestamp) -> pd.Series:
    ...
    if df.empty:
        raise ValueError(describe_missing_fx_history(start, end))
...
def lookup_history_value(
    ...
    *,
    ...
    elif prefer_on_or_before:
        if pos == 0:
            raise ValueError(
                f"No historical value available on or before {normalized_target.date()}; "
                f"earliest available date is {pd.Timestamp(index[0]).date()}."
            )
...
def write_trades_markdown(
    ...
    for r in results:
        ...
        lines.append(f"- Periods: executed {r.periods_executed} / planned {r.periods_planned}")
...
class MainWindow(QMainWindow):
    def __init__(self) -> None:
        ...
        self.period_spin.setMinimumWidth(220)
        self.period_spin.setSizePolicy(QSizePolicy.Policy.Expanding, QSizePolicy.Policy.Fixed)
        ...
        self.amount_spin.setSizePolicy(QSizePolicy.Policy.Expanding, QSizePolicy.Policy.Fixed)
        ...
        self.dividend_mode.setSizePolicy(QSizePolicy.Policy.Expanding, QSizePolicy.Policy.Fixed)
        ...
        self.display_mode.setSizePolicy(QSizePolicy.Policy.Expanding, QSizePolicy.Policy.Fixed)
        ...
        start_field = self.create_stacked_field("Start Date", self.start_date, expanding=False)
        end_field = self.create_stacked_field("End Date", self.end_date, expanding=False)
        period_field = self.create_stacked_field("DCA Period", self.period_spin, expanding=True)
        amount_field = self.create_stacked_field("Amount (NTD)", self.amount_spin, expanding=True)
        dividend_field = self.create_stacked_field("Dividend Mode", self.dividend_mode, expanding=True)
        display_field = self.create_stacked_field("Display Currency", self.display_mode, expanding=True)

        button_panel = QWidget()
        button_panel.setSizePolicy(QSizePolicy.Policy.Fixed, QSizePolicy.Policy.Maximum)
        ...
        button_layout.addWidget(self.run_btn, 0, 0, Qt.AlignmentFlag.AlignTop)
        button_layout.addWidget(self.clear_btn, 1, 0, Qt.AlignmentFlag.AlignTop)
    ...
    def create_stacked_field(self, label_text: str, control: QWidget, *, expanding: bool) -> QWidget:
        field = QWidget()
        field.setSizePolicy(
    ...
    def on_run(self) -> None:
        ...
        if has_foreign_quote_assets:
            try:
                ...
                fx_earliest = pd.Timestamp(usd_twd_history.index[0])
                if start.normalize() < fx_earliest:
                    self.log_box.append(
                        f"[WARN] Historical USD/TWD data starts at {fx_earliest.date()}. "
                        "Foreign-quote assets before this date are not backfilled with a fallback FX rate."
                    )
            except Exception as exc:
